...
- **应用不关也会继续查**：首次检查之后每 6 小时再查一次，所以长期开着 Mareo 的用户当天就能看到新版本，不必重启
- **每天最多提醒一次**（记录在 `<userData>/update-prompt.json`），因此高频复查不会变成打扰；上一个弹窗没回答完之前不会再弹一个
- 若新版本带 `minimumVersion`（强制更新），运行中的提示会换成「必须更新」语气，并说明"可以先用完手头的事，但**下次启动前必须完成更新**"——拦截本身仍只发生在启动时，不会打断正在进行的对话
